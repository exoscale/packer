...
type Config struct {
	...
	// Previous image code. If there is an image previously created, it can be used to create a new image. (server_image_product_code is required if not specified)
	...
	// You can add block storage ranging from 10 GB to 2000 GB, in increments of 10 GB.
	BlockStorageSize                  int    `mapstructure:"block_storage_size" required:"false"`
	// Name of the region where you want to create an image. (default: Korea)
	Region                            string `mapstructure:"region" required:"false"`
	// This is used to allow winrm access when you create a Windows server. An ACG that specifies an access source (0.0.0.0/0) and allowed port (5985) must be created in advance.
