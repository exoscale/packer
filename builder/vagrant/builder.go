...
type Config struct {
	...
	// if your source_box is a boxfile that we need to add to Vagrant, this is the name to give it. If left blank, will default to "packer_" plus your buildname.
	BoxName      string `mapstructure:"box_name" required:"false"`

	// The vagrant provider. This parameter is required when source_path have more than one provider,  or when using vagrant-cloud post-processor. Defaults to unset.
	...
	// Whether to halt, suspend, or destroy the box when the build has completed. Defaults to "halt"
	...
	// a path to a golang template for a vagrantfile. Our default template can be found here. So far the only template variables available to you are {{ .BoxName }} and {{ .SyncedFolder }}, which correspond to the Packer options box_name and synced_folder.
	Template     string `mapstructure:"template" required:"false"`
	SyncedFolder string `mapstructure:"synced_folder"`
	// Don't call "vagrant add" to add the box to your local environment; this is necessary if you want to launch a box that is already added to your vagrant environment.
	SkipAdd     bool   `mapstructure:"skip_add" required:"false"`
	// Equivalent to setting the --cacert option in vagrant add; defaults to unset.
	AddCACert   string `mapstructure:"add_cacert" required:"false"`
	// Equivalent to setting the --capath option in vagrant add; defaults to unset.
	AddCAPath   string `mapstructure:"add_capath" required:"false"`
	// Equivalent to setting the --cert option in vagrant add; defaults to unset.
	AddCert     string `mapstructure:"add_cert" required:"false"`
	// Equivalent to setting the --clean flag in vagrant add; defaults to unset.
	AddClean    bool   `mapstructure:"add_clean" required:"false"`
	// Equivalent to setting the --force flag in vagrant add; defaults to unset.
	AddForce    bool   `mapstructure:"add_force" required:"false"`
	// Equivalent to setting the --insecure flag in vagrant add; defaults to unset.
	AddInsecure bool   `mapstructure:"add_insecure" required:"false"`
	// if true, Packer will not call vagrant package to package your base box into its own standalone .box file.
