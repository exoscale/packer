...
type Config struct {
	...
	// The JSON file containing your account credentials. Not required if you run Packer on a GCE instance with a service account. Instructions for creating the file or using service accounts are above.
	...
	// Full or partial URL of the guest accelerator type. GPU accelerators can only be used with "on_host_maintenance": "TERMINATE" option set. Example: "projects/project_id/zones/europe-west1-b/acceleratorTypes/nvidia-tesla-k80"
	...
	// The name of a pre-allocated static external IP address. Note, must be the name and not the actual IP address.
	Address                      string                         `mapstructure:"address" required:"false"`
	// If true, the default service account will not be used if service_account_email is not specified. Set this value to true and omit service_account_email to provision a VM with no service account.
	...
	// Type of disk used to back your instance, like pd-ssd or pd-standard. Defaults to pd-standard.
	DiskType                     string                         `mapstructure:"disk_type" required:"false"`
	// The unique name of the resulting image. Defaults to "packer-{{timestamp}}".
	ImageName                    string                         `mapstructure:"image_name" required:"false"`
	// The description of the resulting image.
	ImageDescription             string                         `mapstructure:"image_description" required:"false"`
	ImageEncryptionKey           *compute.CustomerEncryptionKey `mapstructure:"image_encryption_key"`
	// The name of the image family to which the resulting image belongs. You can create disks by specifying an image family instead of a specific image name. The image family always returns its latest image that is not deprecated.
	ImageFamily                  string                         `mapstructure:"image_family" required:"false"`
	ImageLabels                  map[string]string              `mapstructure:"image_labels"`
	ImageLicenses                []string                       `mapstructure:"image_licenses"`
	// A name to give the launched instance. Beware that this must be unique. Defaults to "packer-{{uuid}}".
	...
	// A Minimum CPU Platform for VM Instance. Availability and default CPU platforms vary across zones, based on the hardware available in each GCP zone. Details
	MinCpuPlatform               string                         `mapstructure:"min_cpu_platform" required:"false"`
	// The Google Compute network id or URL to use for the launched instance. Defaults to "default". If the value is not a URL, it will be interpolated to projects/((network_project_id))/global/networks/((network)). This value is not required if a subnet is specified.
	Network                      string                         `mapstructure:"network" required:"false"`
	// The project ID for the network and subnetwork to use for launched instance. Defaults to project_id.
	NetworkProjectId             string                         `mapstructure:"network_project_id" required:"false"`
	// If true, the instance will not have an external IP. use_internal_ip must be true if this property is true.
	OmitExternalIP               bool                           `mapstructure:"omit_external_ip" required:"false"`
	// Sets Host Maintenance Option. Valid choices are MIGRATE and TERMINATE. Please see GCE Instance Scheduling Options, as not all machine_types support MIGRATE (i.e. machines with GPUs). If preemptible is true this can only be TERMINATE. If preemptible is false, it defaults to MIGRATE
	...
	// The region in which to launch the instance. Defaults to the region hosting the specified zone.
	Region                       string                         `mapstructure:"region" required:"false"`
	Scopes                       []string                       `mapstructure:"scopes"`
	// The service account to be used for launched instance. Defaults to the project's default service account unless disable_default_service_account is true.
	...
	// The path to a startup script to run on the VM from which the image will be made.
	StartupScriptFile            string                         `mapstructure:"startup_script_file" required:"false"`
	// The Google Compute subnetwork id or URL to use for the launched instance. Only required if the network has been created with custom subnetting. Note, the region of the subnetwork must match the region or zone in which the VM is launched. If the value is not a URL, it will be interpolated to projects/((network_project_id))/regions/((region))/subnetworks/((subnetwork))
	Subnetwork                   string                         `mapstructure:"subnetwork" required:"false"`
	Tags                         []string                       `mapstructure:"tags"`
	// If true, use the instance's internal IP instead of its external IP during building.
